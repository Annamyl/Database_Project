...
cat = ["Vegetarian", "Healthy sweet", "Carbs", "Dairy", "Legumes", "Beef", "Chicken", "Egg", "Seafood", "Oil"]

for i in range(10):
    image = fake.image_url()
    image_desc = fake.text(max_nb_chars=150)
    script += f'INSERT INTO food_groups (group_name, rec_category, summary, group_image, image_description) VALUES ("{food_groups[i]}", "{cat[i]}", "{description[i]}", "{image}", "{image_desc}");\n'
...
for id in range(1, Ingr_const+1):
    ...
    calories = random.randint(10, 150)
    image = fake.image_url()
    image_desc = fake.text(max_nb_chars=150)
...
for id in range(1, Recipes_const+1):
    ...
    for i in range(n):
        ...
        if s == 1:
            q = str(random.randint(1,5)) + ' ' + fake.measurement()
        elif s == 2:
            t = fake.metric_measurement()
            if t == 'liter':
                q = str(random.randint(1,3)) + ' ' + t
            else:
                q = str(random.randint(10,300)) + ' ' + t
        else:
            q = fake.measurement_size()
        script += f'INSERT INTO recipe_ingredients (recipe_id, ingredient_id, main_ingr, quantity) VALUES ({id}, {j}, {bit}, "{q}");\n'
